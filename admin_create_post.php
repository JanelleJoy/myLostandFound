...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($type) || empty($description) || empty($location)) {
        ...
        if (!$error) {
            $stmt = $conn->prepare("INSERT INTO admin_post (title, type, description, location, image_path, admin_email, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("ssssss", $title, $type, $description, $location, $image_path, $admin_email);
...
<head>
<meta charset="UTF-8" />
<title>Admin: Create Lost/Found Post</title>
...
<form method="POST" enctype="multipart/form-data" novalidate>
  ...
  <select name="type" id="type" required>
    ...
    <option value="Found" <?= (isset($_POST['type']) && $_POST['type'] === 'Found') ? 'selected' : '' ?>>Found</option>
        <option value="Surrender" <?= (isset($_POST['type']) && $_POST['type'] === 'Found') ? 'selected' : '' ?>>Surrender</option>
